...
//      .then(() => forever ? this.tailForever() : this.tail())
  ...
  cleanIntermediateTables() {
      util.log('Begin to remove tables...')
      let promise = Promise.resolve()
      promise = promise.then(() => this.mysql.cleanIntermediateTables());

      promise.then(() => {
        util.log('Remove done..')
      })
    ...
    return new Promise((resolve, reject) =>
      mongodb.connect(this.url, function (err, db) {
        if (err) {
          ...
          const stream = db.collection(def.name).find(def.condition).stream();
          stream
            .on('data', item => {
              try{
                stream.pause()
            ...
            .on('end', () => {
              self.mysql.insertDone(() => { util.log(`colltected insert done`) })
              resolve()
            })
        }
      })
