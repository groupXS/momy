...
class Tailer {
  ...
  importAndStart(forever) {
    ...
    this.mysql.createTable()
      .then(() => this.mysql.createPositionTable())
      .then(() => this.importAll())
      .then(() => this.renameTables())
      .then(() => this.createViews())
      .then(() => this.stop('no error found - quit'))
...
//      .then(() => forever ? this.tailForever() : this.tail())
  ...
  importAll() {
    ...
    promise = promise.then(() => this.importPositions());
  ...
  renameTables() {
    util.log('Begin to rename tables...')
    let promise = Promise.resolve()
    promise = promise.then(() => this.mysql.renameTables());
    promise = promise.then(() => this.mysql.renamePositionTable());

    promise.then(() => {
      util.log('Rename done..')
    })
    ...
    return new Promise((resolve, reject) =>
      mongodb.connect(this.url, function (err, db) {
        if (err) {
          ...
          util.log(`auth done starting stream positions`);
          const stream = db.collection('datapayloads').aggregate([{$match:{"payload.type":"fahrenholz_moved", "payload.data.endPos": {$exists:  true}}} , { $sort: { "payload.order": 1 } },{$group:{_id: "$payload.protocolId", endPos:{$last:"$payload.data.endPos"}, summary:{$last: "$payload.data.summary"}}},{ "$project": {position: { $cond: { if: {"$gt": ["$summary"  , null ]}, then: "$endPos", else: "unterwegs" } }}}]).stream();
          stream
            .on('data', item => {
              try{
                stream.pause()
                self.mysql.insertPosition(item, () => stream.resume())
              }catch(error){
                reject(error);
              }
            })
