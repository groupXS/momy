...
class MySQL {
  ...
  constructor (url, defs, views) {
    ...
    this.con = null
    this.tablePostfix = '_' + moment().unix();
  }
  ...
  insert (def, item, replaceFlag, callback) {
    ...
    if(insert){
      const sql = `${command} INTO \`${def.distName}${this.tablePostfix}\`` +
...
insertPosition (item, callback) {
      const sql = `INSERT INTO Positions${this.tablePostfix}` +
  ...
  createTable () {
    ...
    const sql2 = this.defs.map(def => {
      ...
      return `DROP TABLE IF EXISTS \`${def.distName}${this.tablePostfix}\`; ` +
        `CREATE TABLE \`${def.distName}${this.tablePostfix}\` (${fields.join(', ')}) CHARACTER SET = utf8;`
    }).join('')
  ...
  renameTables () {
    let sql = this.defs.map(def => {
      return `DROP TABLE IF EXISTS \`${def.distName}\`; ` + `ALTER TABLE \`${def.distName}${this.tablePostfix}\` RENAME TO \`${def.distName}\`;`
    }).join('');
  ...
  createPositionTable(){
   return this.query(`DROP TABLE IF EXISTS Positions${this.tablePostfix}; CREATE TABLE Positions${this.tablePostfix} (protocolId varchar(255), position varchar(255)) CHARACTER SET = utf8; DROP VIEW IF EXISTS PositionsView;` 
    + `CREATE VIEW PositionsView AS SELECT *  FROM Protocols a, Positions b WHERE a.id = b.protocolId`);
  }

  renamePositionTable () {
    let tablename = 'Positions';
    let sql =  `DROP TABLE IF EXISTS \`${tablename}\`; ` + `ALTER TABLE \`${tablename}${this.tablePostfix}\` RENAME TO \`${tablename}\`;`
